...
var layout_templates = [
	# 标准布局
	{r = [3,4,7,8,11,12,15,16,19,20], c = [2,6,10,14,18,22]},
	# 密集布局
	{r = [2,3,6,7,10,11,14,15,18,19], c = [4,8,12,16,20]},
	# 稀疏布局
	{r = [5,6,10,11,15,16,20,21], c = [3,7,11,15,19,23]},
]
...
func start_game():
	grid = []; enemies = []; bullets = []; explosions = []
...
func _build_ui():
	...
	container.clip_contents = true; container.mouse_filter = Control.MOUSE_FILTER_PASS
	container.position = Vector2(20, 10); container.size = Vector2(GRID_W, GRID_H)
	...
	score_label.position = Vector2(ox, 40); score_label.size = Vector2(120, 20)
	...
	high_label.position = Vector2(ox, 60); high_label.size = Vector2(120, 20)
	high_label.add_theme_font_size_override("font_size", 11)
	high_label.add_theme_color_override("font_color", Color(0.7, 0.7, 0.5))
	panel.add_child(high_label)

	wave_label = Label.new()
	wave_label.position = Vector2(ox, 80); wave_label.size = Vector2(120, 20)
	...
	info_label.position = Vector2(ox, 110); info_label.size = Vector2(120, 90)
...
func _build_map():
	...
	var tmpl = layout_templates[(wave - 1) % layout_templates.size()]
	for r in tmpl.r:
		for c in tmpl.c:
		...
		for c in range(COLS):
			...
			if t != Tile.EMPTY:
				...
				match t:
					...
					Tile.BASE: cr.color = Color(0.9, 0.8, 0.2)
				panel.add_child(cr)
...
func _unhandled_input(ev):
	...
	if ev is InputEventMouseButton and ev.pressed:
		if ev.button_index == MOUSE_BUTTON_WHEEL_UP: _player_shoot()
		elif ev.button_index == MOUSE_BUTTON_WHEEL_DOWN: _player_shoot()

func next_wave():
	wave += 1
	enemies_per_wave = 2 + wave * 2
	enemies_spawned = 0; enemies_killed = 0
	spawn_interval = max(0.8, 2.0 - wave * 0.08)
	_build_map(); _draw_map()
...
func _process(delta):
	...
	if player != null and move_timer <= 0:
		...
		if dir >= 0:
			player.dir = dir
			var d = dirs[dir]
			var nx = player.x + d.x; var ny = player.y + d.y
			if nx >= 0 and nx < COLS and ny >= 0 and ny < ROWS and grid[ny][nx] == Tile.EMPTY:
				var blocked = _tank_at(nx, ny)
				if not blocked:
					player.x = nx; player.y = ny; move_timer = move_interval
			_draw_player()
	spawn_timer -= delta
	if spawn_timer <= 0 and enemies_spawned < enemies_per_wave:
		spawn_timer = spawn_interval; _spawn_enemy()
	for e in enemies:
		e.move_timer -= delta; e.shoot_timer -= delta
		if e.move_timer <= 0:
			e.move_timer = 0.2 + randf() * 0.3
			if randf() < 0.4: e.dir = randi() % 4
			var d = dirs[e.dir]; var nx = e.x + d.x; var ny = e.y + d.y
			if nx >= 0 and nx < COLS and ny >= 0 and ny < ROWS and grid[ny][nx] == Tile.EMPTY:
				if not _tank_at(nx, ny): e.x = nx; e.y = ny
				else: e.dir = randi() % 4
			else: e.dir = randi() % 4
		if e.shoot_timer <= 0:
			e.shoot_timer = 0.8 + randf() * 0.8
			_fire_bullet(e, false)
		_draw_enemy(e)
	bullet_move_timer -= delta
	var bullet_ready = bullet_move_timer <= 0
	if bullet_ready: bullet_move_timer = bullet_move_interval
	var new_bullets = []
	for b in bullets:
		if bullet_ready:
			b.timer -= delta; if b.timer <= 0: continue
			var d = dirs[b.dir]; b.x += d.x; b.y += d.y
			if bullet_labels.has(b.id): bullet_labels[b.id].position = Vector2(20 + b.x * CELL, 10 + b.y * CELL - 4)
			...
			if b.is_player_bullet:
				...
				for e in enemies:
					if abs(e.x - b.x) < 1 and abs(e.y - b.y) < 1:
						...
						enemies.erase(e); enemies_killed += 1; score += 100 + wave * 10
				...
				if player != null and abs(player.x - b.x) < 1 and abs(player.y - b.y) < 1:
					if player.invincible_timer <= 0:
						player.hp -= 1; player.invincible_timer = 1.5; _add_explosion(player.x, player.y)
						if player.hp <= 0: player = null; game_over = true; _game_over_msg("💥 坦克被摧毁！")
						else: _draw_player()
					if bullet_labels.has(b.id): bullet_labels[b.id].queue_free(); bullet_labels.erase(b.id)
					continue
	...
	for bid in bullet_labels.keys():
		if not bid in active_ids: bullet_labels[bid].queue_free(); bullet_labels.erase(bid)
	if enemies_killed >= enemies_per_wave and enemies.size() == 0:
		wave_timer += delta
		if wave_timer > 1.5: wave_timer = 0; next_wave()
	_update_ui()
...
func _spawn_enemy():
	...
	var e = {x = s.x, y = s.y, dir = 2, hp = 1, move_timer = 0.2, shoot_timer = 1.0, id = id}
	enemies.append(e); enemies_spawned += 1
	var lbl = Label.new()
	lbl.text = "🔻"; lbl.position = Vector2(20 + e.x * CELL, 10 + e.y * CELL - 4)
	lbl.size = Vector2(CELL, CELL); lbl.add_theme_font_size_override("font_size", 20)
	panel.add_child(lbl); enemy_labels[id] = lbl

func _player_shoot():
	if player == null or shoot_timer > 0: return
	shoot_timer = shoot_cooldown; _fire_bullet(player, true)
...
func _fire_bullet(src, is_player):
	...
	bullet_id_counter += 1
	var d = dirs[src.dir]; b.x += d.x; b.y += d.y
	if b.x >= 0 and b.x < COLS and b.y >= 0 and b.y < ROWS:
		bullets.append(b)
		var lbl = Label.new()
		lbl.text = "●"; lbl.position = Vector2(20 + b.x * CELL, 10 + b.y * CELL - 4)
		lbl.size = Vector2(CELL, CELL); lbl.add_theme_font_size_override("font_size", 14)
		lbl.add_theme_color_override("font_color", Color(1,1,0) if is_player else Color(1,0,0))
		panel.add_child(lbl); bullet_labels[b.id] = lbl

func _add_explosion(x, y):
	var lbl = Label.new()
	lbl.text = "💥"; lbl.position = Vector2(20 + x * CELL, 10 + y * CELL)
	lbl.size = Vector2(CELL, CELL); lbl.add_theme_font_size_override("font_size", 18)
	panel.add_child(lbl)
	var t = Timer.new(); t.wait_time = 0.3; t.one_shot = true
...
func _draw_enemy(e):
	...
	if lbl != null: lbl.position = Vector2(20 + e.x * CELL, 10 + e.y * CELL - 4)

func _game_over_msg(msg):
	_info_msg(msg)
	if score > high_score: high_score = score
	var go = Label.new()
	go.text = "按 Enter 或 R 重新开始"
	go.add_theme_font_size_override("font_size", 14)
	go.add_theme_color_override("font_color", Color(0.8, 0.8, 0.8))
	go.position = Vector2(panel.size.x/2 - 100, panel.size.y/2 + 20)
	go.size = Vector2(200, 24); go.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	panel.add_child(go)

func _info_msg(msg):
	var lbl = Label.new()
	lbl.text = msg; lbl.add_theme_font_size_override("font_size", 20)
...
func _update_ui():
	...
	high_label.text = "最高: " + str(high_score)
	if player != null: score_label.text = "得分: " + str(score) + "  ❤x" + str(player.hp)
