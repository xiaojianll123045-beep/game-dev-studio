# 游戏开发商 / Game Developer Studio

一个以独立游戏开发为主题的经营模拟/沙盒策略游戏。
An indie game dev business sim / sandbox strategy game.

Built with Godot 4.7 (Mono / C# .NET 8).

---

## 开源说明 / Open Source

本游戏已开源。你可以自由地：
This game is open source. You are free to:

- ✅ **游玩 / Play** — 随意玩，无需付费
- ✅ **学习 / Learn** — 读代码、学 Godot C#
- ✅ **修改 / Modify** — Fork、改代码、做你自己的版本
- ✅ **分发 / Distribute** — 重新发布，只要遵守 GPL v3
- ✅ **做 Mod / Make Mods** — Mod 不需要开源（详见下方）

### 许可证 / License

游戏核心代码以 **GNU General Public License v3 (GPL v3)** 发布。

**但 Mod 不受限：** 你为本游戏制作的 Mod（包括脚本、数据包、资源包、插件等）**不需要开源**，可以用任何你喜欢的许可证发布。详见 `LICENSE` 文件中的 Mod Exception 条款。

The game's core source code is licensed under **GPL v3**.
However, **Mods are exempt**: any Mod you create (scripts, data packs, assets, plugins) does **not** need to be open source. See the Mod Exception in `LICENSE`.

---

## Mod 开发 / Mod Development

Mod 系统已就绪，支持：
- **数据 Mod** — 修改数值、配方、事件等
- **语言 Mod** — 添加/修改翻译
- **脚本 Mod** — 用 Godot GDScript 写逻辑扩展
- **Mod API** — `ModAPI` / `ModBridge` 提供丰富的交互接口

详细文档见 `mod_docs/` 目录。
Mod examples 见 `mod_examples/` 目录。

---

## 构建 / Build

```bash
# 确保安装了 .NET 8 SDK 和 Godot 4.7 Mono
# 然后用 Godot 打开 project.godot 即可
```

---

## 联系方式 / Contact

- 开发者 / Developer: 小坚来了
- QQ: 1099155831
- 邮箱 / Email: xjllqw@163.com
- QQ群 / Group: 495063635

---

## 致谢 / Credits

- Godot Engine — 开源游戏引擎
- **DeepSeek** — AI 辅助开发 / AI-assisted development
- 所有 Mod 作者和贡献者
- 每一位玩家 ❤️

*玩得开心！Have fun!*
